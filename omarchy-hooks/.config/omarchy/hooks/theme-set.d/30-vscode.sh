...
if [[ -f "$THPM_CURRENT_THEME_DIR/vscode.json" ]]; then
    ...
    ext="$(jq -r '.extension // empty' "$THPM_CURRENT_THEME_DIR/vscode.json")"
    ext_src="$THPM_CURRENT_THEME_DIR/vscode-extension"
    shopt -s nullglob ; prebuilt=("$THPM_CURRENT_THEME_DIR"/*.vsix) ; shopt -u nullglob
